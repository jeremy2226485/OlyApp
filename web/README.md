...
- `js/state.js` — holds the in-progress session (draft, pre-Start / started, in-progress) in `localStorage` (not `sessionStorage` — see "Current workout" below) across the New Session → Session Detail hand-off.
...
- **5 work sets per main lift** (Cara confirmed the set *count* was never the issue — some sets just had too many reps). `buildPrimaryLift` produces one unified, continuously ascending `sets` array per lift — empty bar, then the warm-up ramp, then 5 work sets spread evenly across the day's target percent range — rendered as a *single* list (`.set-list`) rather than two separate ones, with work-set rows styled distinctly (bold/accent) from ramp rows. **Reps taper down as % climbs** (`repsForPercent()`: 5 reps below 70%, down to 1 rep at 90%+), applied uniformly across ramp and work sets — this is the actual fix for "too many reps," not the set count.
- **Weights, not just percentages, once a max is on file.** Every step in that unified list — ramp and work sets alike — resolves to an actual total weight plus per-side plate math (`(totalWeight - barWeight) / 2`, `perSideWeight()`) — e.g. "90 kg (37.5 kg/side) x1 — 90%" instead of just "90%". Falls back to percent-only display when no max is on file yet. Bar weight is a constant in `generator.js` (`BAR_WEIGHT`, currently 15kg / 33lb for Cara's bar) — change it there if the bar changes.
- **Hack Squat** added to the accessory bank (`posteriorChain` category, alongside Leg Press).
- **Current workout survives switching tabs/apps and reloads.** The in-progress session used to live in `sessionStorage`, which iOS can clear more aggressively for an installed home-screen PWA than a regular Safari tab — switching to another tab (e.g. Maxes) and back could silently lose the generated workout. It's now in `localStorage`, same durability as sessions/maxes.
- **Draft → Start Workout → Current Workout.** A freshly generated session is a `status: "draft"` — still fully editable (swap lifts/exercises, enter maxes). Tapping **Start Workout** (`startCurrentSession()` in `state.js`) locks it to `status: "started"`: the swap controls disappear (no more accidental changes mid-workout), and the primary action becomes **Mark Complete** (with a **Discard Workout** escape hatch, confirm-gated). Home always shows a way back in — "Review Generated Session" for a draft, "▶ Continue Workout" for a started one — instead of "+ New Session," and New Session itself refuses to generate a fresh session over a *started* one (redirects to Continue Workout instead) to avoid silently discarding an active workout. "Save for later" doesn't need a separate action — once started, it's already durably saved; walking away and coming back later just works.

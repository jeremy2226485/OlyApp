...
- **3 work sets per main lift, not 5.** `buildPrimaryLift`'s `workSets` is an array of 3 sets evenly spread across the day's target percent range (e.g. 75% / 82.5% / 90%), replacing the old 5-set text-only description.
- **Weights, not just percentages, once a max is on file.** Every work set and every build-up ramp step resolves to an actual total weight plus per-side plate math (`(totalWeight - barWeight) / 2`) — e.g. "90 kg (37.5 kg/side)" instead of just "90%". Falls back to percent-only display when no max is on file yet, same as before. Bar weight is a constant in `generator.js` (`BAR_WEIGHT`, currently 15kg / 33lb for Cara's bar) — change it there if the bar changes.
